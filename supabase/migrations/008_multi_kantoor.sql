-- Migration: 008_multi_kantoor
-- Implements multi-kantoor architecture.
-- Preserves backward compatibility: rows with kantoor_id = NULL remain accessible
-- by their original user_id owner (legacy access pattern).

-- ---------------------------------------------------------------------------
-- New tables: kantoren + kantoor_members
-- ---------------------------------------------------------------------------
...
CREATE POLICY "Kantoor members can view their kantoor" ON kantoren
  FOR SELECT USING (
    id IN (SELECT km.kantoor_id FROM kantoor_members km WHERE km.user_id = auth.uid())
  );

CREATE POLICY "Authenticated users can create kantoren" ON kantoren
  FOR INSERT WITH CHECK (true);

-- ---------------------------------------------------------------------------
-- RLS for kantoor_members
-- ---------------------------------------------------------------------------
...
CREATE POLICY "Kantoor members can view members of their kantoor" ON kantoor_members
  FOR SELECT USING (
    kantoor_id IN (SELECT km.kantoor_id FROM kantoor_members km WHERE km.user_id = auth.uid())
  );
...
CREATE POLICY "Users can insert own membership" ON kantoor_members
...
-- ---------------------------------------------------------------------------
-- Add kantoor_id column to all data tables
-- ---------------------------------------------------------------------------
...
ALTER TABLE feedback_samenvattingen ADD COLUMN IF NOT EXISTS kantoor_id UUID REFERENCES kantoren(id);
ALTER TABLE gebruiker_schrijfprofiel ADD COLUMN IF NOT EXISTS kantoor_id UUID REFERENCES kantoren(id);
...
CREATE INDEX IF NOT EXISTS feedback_samenvattingen_kantoor_id_idx   ON feedback_samenvattingen (kantoor_id);
CREATE INDEX IF NOT EXISTS gebruiker_schrijfprofiel_kantoor_id_idx  ON gebruiker_schrijfprofiel (kantoor_id);

-- UNIQUE constraint for similarity_instellingen per kantoor (NULL excluded via partial index)
CREATE UNIQUE INDEX IF NOT EXISTS similarity_instellingen_kantoor_id_unique
  ON similarity_instellingen (kantoor_id) WHERE kantoor_id IS NOT NULL;

-- ---------------------------------------------------------------------------
-- Rewrite RLS policies — allow legacy (kantoor_id IS NULL) rows to remain
-- accessible by the original user_id owner.
-- ---------------------------------------------------------------------------
...
CREATE POLICY "Kantoor members can access dossiers" ON dossiers
  ...
  USING (
    kantoor_id IN (SELECT km.kantoor_id FROM kantoor_members km WHERE km.user_id = auth.uid())
    OR (kantoor_id IS NULL AND user_id = auth.uid())
  )
  WITH CHECK (
    kantoor_id IN (SELECT km.kantoor_id FROM kantoor_members km WHERE km.user_id = auth.uid())
    OR (kantoor_id IS NULL AND user_id = auth.uid())
...
-- sectie_feedback
DROP POLICY IF EXISTS "Users manage own sectie feedback" ON sectie_feedback;
...
-- ai_usage_log — keep the service-role insert policy, replace the user read policy
DROP POLICY IF EXISTS "Users read own ai_usage_log" ON ai_usage_log;

CREATE POLICY "Kantoor members can read ai_usage_log" ON ai_usage_log
  FOR SELECT
  USING (
    kantoor_id IN (SELECT km.kantoor_id FROM kantoor_members km WHERE km.user_id = auth.uid())
    OR (kantoor_id IS NULL AND user_id = auth.uid())
...
-- feedback_samenvattingen — replace 3 separate policies with one combined policy
DROP POLICY IF EXISTS "Users can read own feedback summaries" ON feedback_samenvattingen;
DROP POLICY IF EXISTS "Users can insert own feedback summaries" ON feedback_samenvattingen;
DROP POLICY IF EXISTS "Users can update own feedback summaries" ON feedback_samenvattingen;

CREATE POLICY "Kantoor members can access feedback_samenvattingen" ON feedback_samenvattingen
  FOR ALL
  USING (
    kantoor_id IN (SELECT km.kantoor_id FROM kantoor_members km WHERE km.user_id = auth.uid())
    OR (kantoor_id IS NULL AND user_id = auth.uid())
...
-- gebruiker_schrijfprofiel — replace 3 separate policies with one combined policy
DROP POLICY IF EXISTS "Users can read own schrijfprofiel" ON gebruiker_schrijfprofiel;
DROP POLICY IF EXISTS "Users can insert own schrijfprofiel" ON gebruiker_schrijfprofiel;
DROP POLICY IF EXISTS "Users can update own schrijfprofiel" ON gebruiker_schrijfprofiel;

CREATE POLICY "Kantoor members can access gebruiker_schrijfprofiel" ON gebruiker_schrijfprofiel
  FOR ALL
  USING (
    kantoor_id IN (SELECT km.kantoor_id FROM kantoor_members km WHERE km.user_id = auth.uid())
    OR (kantoor_id IS NULL AND user_id = auth.uid())
...
-- ---------------------------------------------------------------------------
-- Update match_document_chunks RPC to support kantoor filtering
-- ---------------------------------------------------------------------------
